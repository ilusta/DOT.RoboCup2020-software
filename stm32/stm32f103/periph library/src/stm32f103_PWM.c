...
void initPWM(unsigned int timN, uint8_t channel, GPIO_TypeDef * prt, uint16_t pinNum, unsigned int freq, unsigned int clk){		//freq [Hz], clk [MHz]
	...
	tim->PSC = (clk * 1000000 / freq / 1000) - 1;			//calculate frequency
	tim->ARR = 1000;																	//timer counts from 0 to 1000	
	tim->CR1 |= TIM_CR1_ARPE;
	
	switch(channel){
		case 1:
			tim->CCR1 = 500;																				//start with 50%
